...
def make_env(env_name: str, seed: int = 0, post_success_steps: int = 0):
    ...
    adapter but ignored — Adroit episodes have no follow-through window after
    success.
    """
...
def get_dataset(env_name: str):
    """Load a Minari dataset and convert it into the project's Dataset format."""
    ...
    for episode in tqdm(dataset, desc=f"Processing '{dataset_id}'"):
        ...
        # Drop the final post-termination observation: N observations for N actions.
    ...
    # At terminal states, next_observation should equal the current observation.
