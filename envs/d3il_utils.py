...
def quat2euler(quat):
    ...
    # Simple conversion - assuming quat is [w, x, y, z] or [x, y, z, w]
    # For yaw extraction from quaternion
    if len(quat) == 4:
        # Extract yaw (rotation around z-axis)
        # quat format: [x, y, z, w] or [w, x, y, z]
        ...
        # Roll and pitch (not used here but for completeness)
        sinr_cosp = 2 * (w * x + y * z)
...
def flatten_obs_dict(obs):
    """
    Flattens observation into a single numpy array.
    For D3IL, obs is already a flat array from the environment.
    """
    if isinstance(obs, dict):
        # If obs is a dict, flatten it
        parts = []
...
class D3ILWrapper(gym.Wrapper):
    """
    Wrapper for D3IL environments to match Franka Kitchen interface.
    """
    def __init__(self, env):
        super().__init__(env)
        # D3IL environments return flat observations
        # No need to modify observation_space
    ...
    def reset(self, **kwargs):
        ...
        # Initialize tracking (like Franka Kitchen)
        info['success'] = 0.0
    ...
    def step(self, action):
        ...
        # Ensure success flag is present (like Franka Kitchen)
        if 'success' not in info:
            info['success'] = float(terminated and reward > 0.5)
        
        # Return in gymnasium format (5-tuple)
        return obs, reward, terminated, truncated, info


def make_env(env_name: str, seed: int = 0, **kwargs) -> gym.Env:
    """
    Creates a D3IL environment and applies wrapper.
    
    Args:
        env_name: Either 'gate_insertion-state', 'gate_insertion-image', 
                  'stacking-state', or 'stacking-image'
    """
...
def build_observation_stacking(episode_data: Dict, t: int) -> np.ndarray:
    """
    Build observation for stacking task matching the environment's get_observation().
    
    Observation structure (Corrected):
    - j_state (joint state)
    - robot_c_pos (cartesian position)
    - robot_c_quat (cartesian quaternion)
    - red_box_pos (3D)
    - red_box_orient (2D - cos(yaw), sin(yaw))
    - green_box_pos (3D)
    - green_box_orient (2D - cos(yaw), sin(yaw))
    - blue_box_pos (3D)
    - blue_box_orient (2D - cos(yaw), sin(yaw))
    """
    robot_data = episode_data['robot']
    
    obs_parts = []
    
    # j_state (joint positions and velocities + gripper)
...
def build_observation_insertion(episode_data: Dict, t: int) -> np.ndarray:
    """
    Build observation for insertion task matching the environment's get_observation().
    
    Observation structure (Corrected):
    - robot_pos[:2] (first 2 elements of robot state)
    - box1_pos[:2] (only x,y)
    - box1_orient (2D - cos(yaw), sin(yaw))
    - box2_pos[:2]
    - box2_orient (2D - cos(yaw), sin(yaw))
    - box3_pos[:2]
    - box3_orient (2D - cos(yaw), sin(yaw))
    - target_box1_pos[:2]
    - target_box2_pos[:2]
    - target_box3_pos[:2]
    """
    ...
    # robot_pos[:2] - first 2 elements of robot state (likely j_pos[:2])
...
def get_dataset(env_name: str, data_dir: Optional[str] = None) -> Dataset:
    """
    Loads and processes the D3IL dataset.
    
    Args:
        env_name: Environment name like 'gate_insertion-state' or 'stacking-state'
        data_dir: Path to dataset directory
    
    Returns:
        Dataset object compatible with OGPO pipeline
    """
    # Parse environment name
    if '-state' in env_name:
        task_name = env_name.replace('-state', '')
    ...
    # Map task names to data directories
    task_to_data_dir = {
    ...
    # Build sample observation to get shape
    ...
    # Build sample action (des_j_pos + gripper for stacking)
    ...
    for filepath in tqdm(valid_files, desc=f"Processing '{task_name}'"):
        try:
            ...
            # Build observations for each timestep (matching environment's get_observation)
            obs_list = []
            ...
            # Build actions (des_j_pos + gripper for stacking)
            action_list = []
            for t in range(traj_len):
                action_parts = []
                action_parts.append(robot_data['des_j_pos'][t])
                
                # Add gripper for stacking
                if 'gripper_width' in robot_data:
                    gripper = robot_data['gripper_width'][t]
                    if np.isscalar(gripper):
                        action_parts.append(np.array([gripper]))
            ...
            # DEBUG: Print statistics for this episode
            ...
            # Rewards (sparse: 1.0 at end if successful)
            ...
            if 'context' in episode_data:
                ...
                # Assume successful demonstration
                episode_rewards[-1] = 1.0
            
            # Terminals (only last step)
    ...
    # Create next_observations (like Franka Kitchen)
