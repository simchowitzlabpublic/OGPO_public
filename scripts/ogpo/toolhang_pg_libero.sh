...
# === Config ===
# Experimental: PaliGemma backbone finetuned on LIBERO (pi05_libero), two-tier obs encoder.
# Task: tool_hang (hardest robomimic task; long episodes, discount 0.999, horizon 8).
# Mirrors square_pg_libero.sh (the converging config): two-tier image/proprio fusion,
# tapered SDE noise + Theorem-17 score correction, use_denoiser=false.
seed=82723
device=0
encoder_device=1                # GPU for PaliGemma pre-encoding (separate from training)
jax_frac=0.90  # openpi recommends 0.9 (default 0.75); 0.95 leaves no room for cublas
env_name="tool_hang-ph-image"
run_name="ogpo_tool_hang_pg_libero"
wandb_name="ogpo_tool_hang_pg_libero"
project="OGPO"
run_group="tool_hang_pg_libero"
...
value_hidden_dims="[1024,1024,1024]"
tf_pi_layers=5
tf_pi_embed_dim=128
tf_pi_heads=4
tf_q_layers=5
...
#   full    -> robot proprio + object pose (privileged info — DO NOT use as default)
...
# PaliGemma vision encoder: LIBERO-finetuned Pi0.5 backbone
# Download with: gcloud storage cp -r gs://openpi-assets/checkpoints/pi05_libero/ \
#                     $HOME/checkpoints/
...
# Sanity-check the LIBERO checkpoint exists before launching
if [ ! -d "$restore_encoder_path" ]; then
    echo "ERROR: pi05_libero checkpoint not found at $restore_encoder_path"
    echo "Download with:"
    echo "  gcloud storage cp -r gs://openpi-assets/checkpoints/pi05_libero/ \\"
    echo "      \$HOME/checkpoints/"
    exit 1
fi

echo "=== OGPO Tool-Hang PaliGemma (LIBERO-finetuned, two-tier) | seed=$seed | encoder=$vit_encoder | model=$restore_encoder_path ==="
