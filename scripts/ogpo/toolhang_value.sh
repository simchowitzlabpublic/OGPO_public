...
# =============================================================================
# OGPO + V(s) ablations on tool_hang-ph-low_dim.
#
# Defaults = Ablation A: V trained by off-policy TD, V(s') replaces next_q.
#
# Ablation B (V distilled from mean_a Q):
#   bash scripts/ogpo/toolhang_value.sh --v_training_mode=bc_q_mean --q_variance_reduction=true
#
# Baseline (no V, original OGPO):
#   bash scripts/ogpo/toolhang_value.sh --use_value_fn=false --adv_baseline=group_mean
# =============================================================================
...
run_name="ogpo_toolhang_value"
wandb_name="ogpo_toolhang_value"
project="OGPO"
run_group="toolhang_value_ablation"
...
v_loss_coeff=1.0
v_pair_with_q=false           # If true, supervise V_i per-head (requires v_ensemble_size==num_qs)
...
echo "=== OGPO Toolhang V-ablation | seed=$seed | use_value_fn=$use_value_fn v_training_mode=$v_training_mode adv_baseline=$adv_baseline q_vr=$q_variance_reduction ==="
