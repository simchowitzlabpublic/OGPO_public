...
run_name="ogpo_toolhang"
wandb_name="ogpo_toolhang"
project="OGPO"
run_group="toolhang"
...
# Data augmentation
p_aug=0.0
num_offline_trajs=-1    # -1 = all trajectories; >0 = subsample N full rollouts
...
# CalQL critic-pretraining (only used when calql_steps > 0)
cql_alpha=5.0
cql_temp=1.0
cql_n_actions=10
calql_q_subsample=2
...
python ogpo/main.py \
    ...
    --agent.cql_alpha=$cql_alpha \
    --agent.cql_temp=$cql_temp \
    --agent.cql_n_actions=$cql_n_actions \
    --agent.calql_q_subsample=$calql_q_subsample \
